...
import NekoXRobot.modules.sql.users_sql as sql

from NekoXRobot import (ALLOW_EXCL, CERT_PATH, DONATION_LINK, LOGGER,
                          OWNER_ID, PORT, SUPPORT_CHAT, TOKEN, URL, WEBHOOK,
                          SUPPORT_CHAT, dispatcher, StartTime, telethn, updater, pbot)
# needed to dynamically load modules
# NOTE: Module order is not guaranteed, specify that in the config file!
from NekoXRobot.modules import ALL_MODULES
from NekoXRobot.modules.helper_funcs.chat_status import is_user_admin
from NekoXRobot.modules.helper_funcs.misc import paginate_modules
...
for module_name in ALL_MODULES:
    imported_module = importlib.import_module("NekoXRobot.modules." +
                                              module_name)
...
def get_help(update: Update, context: CallbackContext):
    ...
    if chat.type != chat.PRIVATE:
        ...
        update.effective_message.reply_photo(
            GROUP_HELP_IMG, caption= "Contact me in PM to get the list of possible commands.",
            reply_markup=InlineKeyboardMarkup(
                [
                  [
